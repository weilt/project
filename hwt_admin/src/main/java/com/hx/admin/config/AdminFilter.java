 ...
 */
@WebFilter(urlPatterns = {"/admin/*","/adminHx/*","/model/index"})
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    	...
    	if(ObjectHelper.isEmpty(adminUserCache)) {
    		if(ObjectHelper.x_requested_with(req)){
    			//异步请求
    			if (url.endsWith("/list")||url.equalsIgnoreCase("/main")||url.equalsIgnoreCase("/model/index")){
		...
		if(ObjectHelper.isEmpty(listModule)){
			...
			for (AdminModule adminModule : listModule) {
				if(url.equals(adminModule.getUrl().startsWith("/")?adminModule.getUrl():("/"+adminModule.getUrl()))){
					b = true;
					break;
				}
			}
