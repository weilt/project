...
function ajaxCallback(data){
	data = strToJson(data)
		...
		var  applyTotal = data.data.applyTotal;//总申请
		$("#applyTotal").text(applyTotal);
		var unauditedTotal = data.data.unauditedTotal; //总未处理
		$("#unauditedTotal").text(unauditedTotal);
		var adoptTotal = data.data.adoptTotal; //总通过
		$("#adoptTotal").text(adoptTotal);
		var failedTotal = data.data.failedTotal; //总拒绝
		$("#failedTotal").text(failedTotal);
