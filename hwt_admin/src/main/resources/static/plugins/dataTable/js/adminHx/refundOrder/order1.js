 ...
 * @param dec
 */
function order_details1(order_id, url, dec) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            jsonState(res);
                ...
                //     });
                // });
                var Refund_sum = hwOrderVo.payment - Contract_money;
                ...
                _html += '<table id="z-zwz">                                                                                '
                    ...
                    + '</td>                                                                                               '
                    + '<td id="tow">                                                                                               '
                    + '<span id="payment">'+hwOrderVo.payment+'</span>'
                    + '</td>                                                                                               '
                    + '</tr>                                                                                               '
                    + '<tr>                                                                                               '
                    + '<td id="one" >                                                                                               '
                    ...
                    + '</td>                                                                      '
                    + '<td id="tow">                                                                      '
                    + '<input  type="radio" name="contract_money_sel" id="wu" value="0" checked onclick="wu()"/>无                          '
                    +'<input  type="radio" id="you" name="contract_money_sel"  onclick="dianji()" style="margin-left: 100px;"/>有                          '
                    ...
                    +'<table id="z-break2">                                                               '
                    +'<td>                                                               '
                    +'<button onclick="consent('+order_id+',\'/admin/order/consentOrderRefund\')" id="guider2">确认退款</button>                                                               '
                    +'</td>                                                               '
                    +'<td style="width: 80px;">                                         '
                    +'</td>                                                               '
                    //
                    // +'<td>                                                               '
                    // +'<button onclick="refrund('+order_id+',\'/admin/order/refusedOrderRefund\')" id="guider2">拒绝退款</button>                                                               '
                    // +'</td>                                                               '
...
function refrund(order_id,url) {
    var refund_cause = prompt("请输入拒绝原因", "");
    $.ajax({
        url:url,
        type:"POST",
        data:{order_id:order_id,refund_cause:refund_cause},
        success:function (data) {
            // location.reload([false]);
            // order_details1(order_id ,admin/order/refund/list);
            // window.herf = 'admin/order/refund/list'
            var res = data;
            res = strToJson(res);
            console.log(res)
...
function consent(order_id,url) {

    $.ajax({
        url:url,
        type:"POST",
        data:{order_id:order_id,contract_money:$("#money").val() == undefined?0:$("#money").val()},
        success:function (data) {
            // location.reload([false]);
            // order_details1(order_id ,admin/order/refund/list);
            // window.herf = 'admin/order/refund/list'
            ...
            // jumpToPage('admin/order/refund/list',$('.nav-link'))
    }
    });
}
...
function dianji() {
    ...
    var radio = $("#you");
    if (radio[0].checked){
        $("#sp").append('<input id="money" onkeyup="contentChange()" type="text" style="margin-left: 100px;" value="'+Contract_money+'"/>元')
    }
  // $('#money').val(Contract_money);
}
...
function contentChange() {
    var content= $("#money").val();
    var payment = $("#payment").html();
    $("#content").html(payment-content);
}
