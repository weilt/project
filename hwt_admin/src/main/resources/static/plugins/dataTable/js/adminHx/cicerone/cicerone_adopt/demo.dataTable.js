...
$(function(){
	...
	// 请求地址
	_settings.ro_requestUrl = "adminHx/cicerone_adopt/query";
	// 排序设置
...
//			}
//        },
        {filed:'service_area_city_name', 
        	customHtml : function(rowData,index){
        		return rowData.service_area_city_name==null?"":rowData.service_area_city_name	;
			}
        },
       /* {filed:'phone',
        	customHtml : function(rowData,index){
        ...
        {title:'操作', customHtml:
        	...
        	function(rowData,index){
				var html = '';
				html += '<a onclick="cicerone_info('+rowData.user_id+',\'adminHx/cicerone/info\',\'导游资料-查看（'+rowData.real_name+'）\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>导游资料</div></a>';
//				if (rowData.status==2){
//        			//html += '<a onclick="frozen(2,'+rowData.id+',\'admin/module/frozen\',\'确定要删除模块---'+rowData.name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>冻结</div></a>';
//        			html += '<a onclick="deleteAndRecoveryById(3,'+rowData.id+',\'admin/module/frozen\',\'确定要冻结---'+rowData.real_name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>查看评论</div></a>';
//        		}
				/*html += '<a onclick="webpageEject(\'adminHx/user/userFriend?id='+rowData.id+'\',\'用户好友-查看（'+rowData.account+'）\',\'80%\',\'80%\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>用户好友</div></a>';*/
				return html;
        	}
        }	
    ];
