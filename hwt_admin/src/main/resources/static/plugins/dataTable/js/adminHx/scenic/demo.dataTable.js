...
$(function(){
	...
	_settings.ro_columns =  [
        ...
        {title:'操作', customHtml:
        	...
        	function(rowData,index){
				var html = '';
				html += '<a onclick="scenicInfo('+rowData.spotId+',\'adminHx/scenic/scenicInfo\',\'景点管理-查看（'+rowData.spotName+'）\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>查看详情</div></a>';
				...
				if (rowData.isHide==0){
        			html += '<a onclick="deleteAndRecoveryById(1,'+rowData.spotId+',\'adminHx/scenic/delete\',\'确定要隐藏景点---'+rowData.spotName+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>隐藏</div></a>';
        		}else{
        			html += '<a onclick="deleteAndRecoveryById(0,'+rowData.spotId+',\'adminHx/scenic/delete\',\'确定要恢复景点---'+rowData.spotName+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>恢复</div></a>';
        		}
				if (rowData.isHot==0){
					html += '<a onclick="deleteAndRecoveryById(1,'+rowData.spotId+',\'adminHx/scenic/isHot\',\'确定要热门---'+rowData.spotName+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>热门</div></a>';
				}else{
					html += '<a onclick="deleteAndRecoveryById(0,'+rowData.spotId+',\'adminHx/scenic/isHot\',\'确定要取消热门---'+rowData.spotName+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>取消热门</div></a>';
				}
				if (rowData.isRecommend==0){
					html += '<a onclick="deleteAndRecoveryById(1,'+rowData.spotId+',\'adminHx/scenic/isRecommend\',\'确定要推荐---'+rowData.spotName+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>推荐</div></a>';
				}else{
					html += '<a onclick="deleteAndRecoveryById(0,'+rowData.spotId+',\'adminHx/scenic/isRecommend\',\'确定要取消推荐---'+rowData.spotName+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>取消推荐</div></a>';
				}	
				return html;
        	}
        }	
...
function paramBuffer() {
	_settings.ro_queryParams =  function createParams(){
		...
		if(isNotEmpty($("#paramMap_complete").val()))
			params.paramMap.complete = $("#paramMap_complete").val();
