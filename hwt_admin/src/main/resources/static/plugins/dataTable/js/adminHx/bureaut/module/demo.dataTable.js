        ...
        {filed:'name', 
        	customHtml : function(rowData,index){
        		...
        		if (a<4){
        			...
        			return '<a href="javascript:void(0);" onclick="searchByParent('+rowData.bureau_module_id+',\''+rowData.name+'\')">'+rowData.name+'</a>'
        		}else{
        			return rowData.name;
        		}
			}
        ...
        {title:'操作', customHtml:
        	...
        	function(rowData,index){
				...
				if (rowData.is_hide==1){
        			html += '<a onclick="deleteAndRecoveryById(2,'+rowData.bureau_module_id+',\'adminHx/bureaut/module/delete\',\'确定要删除模块---'+rowData.name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>删除</div></a>';
        		}else{
        			html += '<a onclick="deleteAndRecoveryById(1,'+rowData.bureau_module_id+',\'adminHx/bureaut/module/delete\',\'确定要恢复模块---'+rowData.name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>恢复</div></a>';
        		}
