        ...
        {filed:'name', 
        	customHtml : function(rowData,index){
        		...
        		if (a<4){
        			...
        			return '<a href="javascript:void(0);" onclick="searchByParent('+rowData.id+',\''+rowData.name+'\')">'+rowData.name+'</a>'
        		}else{
        			return rowData.name;
        		}
			}
...
//        {filed:'moduleImage',
//        	customHtml : function(rowData,index){
//        		var moduleImage = rowData.moduleImage == null?"icon ion-ios-home-outline":rowData.moduleImage;
//        		return '<ul class="row icon-list list-unstyled tx-32 mg-b-0" style="margin-left: 0;margin-right: 0;"> <li class="col-3 col-sm-2 col-lg-1 tx-center mg-t-10" ><i class="'+moduleImage+'"></i> </li> <ul>';
//        	}
//        },
        {filed:'description', 
        	customHtml : function(rowData,index){
        ...
        {title:'操作', customHtml:
        	...
        	function(rowData,index){
				...
				if (rowData.isHide==1){
        			html += '<a onclick="deleteAndRecoveryById(2,'+rowData.id+',\'admin/module/delete\',\'确定要删除模块---'+rowData.name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>删除</div></a>';
        		}else{
        			html += '<a onclick="deleteAndRecoveryById(1,'+rowData.id+',\'admin/module/delete\',\'确定要恢复模块---'+rowData.name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>恢复</div></a>';
        		}
