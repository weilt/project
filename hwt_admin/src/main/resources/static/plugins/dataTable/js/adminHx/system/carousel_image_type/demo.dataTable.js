...
$(function(){
	...
	// 请求地址
	_settings.ro_requestUrl = "adminHx/carousel_image_type/query";
	// 排序设置
	...
	_settings.ro_columns =  [
        ...
        {title:'操作', customHtml:
        	...
        	function(rowData,index){
				var html = '';
				if (rowData.is_hide==0){
        			html += '<a onclick="deleteAndRecoveryById(1,'+rowData.attribute_id+',\'adminHx/carousel_image_type/delete\',\'确定要删除分类---'+rowData.attribute_name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>删除</div></a>';
        		}else{
        			html += '<a onclick="deleteAndRecoveryById(0,'+rowData.attribute_id+',\'adminHx/carousel_image_type/delete\',\'确定要恢复分类---'+rowData.attribute_name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>恢复</div></a>';
        		}
				html += '<a onclick="edit_attribute('+rowData.attribute_id+',\'adminHx/carousel_image_type/update\',\'轮播图分类-编辑（'+rowData.attribute_name+'）\',1)" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>修改</div></a>';
				return html;
        	}
        }	
    ];
