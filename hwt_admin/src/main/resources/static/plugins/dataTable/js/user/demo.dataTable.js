        ...
        {title:'操作', customHtml:
        	...
        	function(rowData,index){
				...
				if (rowData.isenable==1){
        			html += '<a onclick="deleteAndRecoveryById(2,'+rowData.user_id+',\'admin/user/delete\',\'确定要禁用用户---'+rowData.user_account+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>删除</div></a>';
        		}else{
        			html += '<a onclick="deleteAndRecoveryById(1,'+rowData.user_id+',\'admin/user/delete\',\'确定要恢复用户---'+rowData.user_account+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>恢复</div></a>';
        		}
