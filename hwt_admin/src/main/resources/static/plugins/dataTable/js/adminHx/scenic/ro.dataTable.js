...
function paginInit(count) {
	...
	if (pages > 5){// 总页数大于5页 则判断当前页的位置信息
		if (_settings.ro_currentPage > 3) {
			//显示第一页
			paginHtml += "<a class='paginate_button' id='page_"+1+"' aria-controls='dynamic-table' tabindex='0' a href='javascript:void(0);' onclick='buttonPage(this,"+1+")'>"+1+"</a>";
			//显示前面三个点
			...
			if (pages - _settings.ro_currentPage > 1) {
				for (var i = _settings.ro_currentPage - 2; i < _settings.ro_currentPage; i++) {
					if(i == _settings.ro_currentPage){
						paginHtml += "<a class='paginate_button current' id='page_"+i+"' aria-controls='dynamic-table' tabindex='0' a href='javascript:void(0);' onclick='buttonPage(this,"+pages+")'>"+i+"</a>";
					}else{
						paginHtml += "<a class='paginate_button' id='page_"+i+"' aria-controls='dynamic-table' tabindex='0' href='javascript:void(0);' onclick='buttonPage(this,"+pages+")'>"+i+"</a>";
					}
					
				}
				for (var i = _settings.ro_currentPage; i <= parseInt(_settings.ro_currentPage)+2; i++) {
					if(i == _settings.ro_currentPage){
						paginHtml += "<a class='paginate_button current' id='page_"+i+"' aria-controls='dynamic-table' tabindex='0' a href='javascript:void(0);' onclick='buttonPage(this,"+pages+")'>"+i+"</a>";
					}else{
						paginHtml += "<a class='paginate_button' id='page_"+i+"' aria-controls='dynamic-table' tabindex='0' href='javascript:void(0);' onclick='buttonPage(this,"+pages+")'>"+i+"</a>";
		...
		if (_settings.ro_currentPage + 2 < pages) {
			paginHtml += "<a class='paginate_button' id='page_"+pages+"' aria-controls='dynamic-table' tabindex='0' href='javascript:void(0);' onclick='buttonPage(this,"+pages+")'>"+pages+"</a>";
		}
	...
	paginHtml +="到<input onblur='jumpPages(this,"+pages+")' type='number' styel='width: 69px;height: 40px;'/>页"
...
//	// 设置当前选中的页码样式
//	//$("#page_"+page).prop("class", "paginate_button current");
	_settings.ro_currentPage = page;
	reloadTable(_myDataTables.table);
