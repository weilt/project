...
public class A {
    ...
    public static void main(String[] args) throws IOException, JSONException {
    	...
    	int[] ids = {/*360,756,1366,758,1368,120424,1380,759,120492,2063,760,757,*/100076,1369};
    	String[] citys ={/*"台北","高雄","花莲","嘉义","金门","九份","垦丁","南投","日月潭","新北市","台东","台南",*/"台湾","台中"};
    	for (int j = 0; j < ids.length; j++) {
    		 //要修改的
            String key = "JZ3ZhUiCVTfv9zEWBdZSRLDbqIhB5OITOnrashg7pMfunMEcfxgTMqRfczR6Lmaf";
            String dbname = "hwt11111";
...
////            String url = "http://120.76.205.241:8000/sight/qyer?cityid=11186&apikey=htDWFQQBdjgaMz3epKEPMacnZNQK9Oy09IUurhjrxiB3IbAa26U2EYzj9BYYcrCN";
            ...
            for (int i = 0; i < 100000; i++) {
            	...
            	try {
                	...
                	e.printStackTrace();
                	System.out.println("城市id"+cityid+"传入的pageToken------------"+pageToken+"=========================");
                    count --;
                    if (count == 0){
                        pageToken++;
                        count = 10;
            ...
            //    System.out.println("返回的pageToken------------"+m.get("pageToken"));
                ...
                if(m != null) {
                    ...
                    for (Map<String, Object> objectMap : l) {
                        ...
                        if (ObjectHelper.isEmpty(ss.getCity())||"[]".equals(ss.getCity())){
                        	ss.setCity(objectMap.get("city") + "");
                        }
                        try {
   ...
   private static void writerText(String filePath,String content){
    ...
    public static boolean is_cha(String url ,int a ){
    	...
    	for (int i = 0; i < 10; i++) {
			...
			try {
				requestFromUrl = getRequestFromUrl(url+a);
				...
				System.out.println("城市id"+url.split("cityid")[1]+"传入的pageToken------------"+a+"=========================");
				e.printStackTrace();
				return is_cha( url ,++a);
			} 
    ...
    private static ScenicSpot createSpot(Map<String,Object> map ) {
        ...
        ss.setCreate_time(currentTimeMillis);
        ss.setDataTime(currentTimeMillis);
        ss.setIsHide(1);
        ss.setVisitsNum(0l);
        ss.setIsHot(0);
        ss.setIsRecommend(0);
