...
import com.hx.core.es.entity.SearchNearby;
...
public class ElasticsearchUtils {
    ...
    public static boolean createMappingESQ(String index,String type)throws Exception{
    	  XContentBuilder builder=XContentFactory.jsonBuilder()
                  ...
                  .startObject("name").field("type", "text").field("store", "yes").field("boost", 5.0).field("analyzer", "ik_max_word").endObject()
                  .startObject("location").field("type", "text").field("store", "yes").field("boost", 3.0).field("analyzer", "ik_max_word").endObject()
                  .startObject("tar").field("type", "text").field("store", "yes").field("boost", 4.0).field("analyzer", "ik_max_word").endObject()
                  ...
                  .startObject("can_not_recommended").field("type", "integer").field("store", "yes").endObject()
                  ...
                  .startObject("author").field("type", "string").field("store", "yes").field("index", "not_analyzed").endObject()
                  .startObject("is_display").field("type", "integer").field("store", "yes").endObject()
                  .startObject("is_recommend").field("type", "integer").field("store", "yes").endObject()
                  .startObject("is_official_recommend").field("type", "integer").field("store", "yes").endObject()
                  .endObject().endObject();
	 ...
	 * 搜附近
	 */
	public static EsPage searchNearbyListData(SearchRequestParam param){

		SearchRequestBuilder searchRequestBuilder;
		try{
			searchRequestBuilder = getSearchRequestBuilder(param);
			if (param.getSearchNearby()!=null){
				searchRequestBuilder = getSearchNearbyRequestBuilder(param.getSearchNearby(),searchRequestBuilder);
			}
	 ...
	 * @return
	 */
	public static SearchRequestBuilder getSearchNearbyRequestBuilder(SearchNearby param, SearchRequestBuilder searchRequestBuilder) {
        //查询某经纬度100米范围内
        double lat = param.getLat();
		double lon = param.getLon();
		String filed = param.getFiled();
		Integer sort = param.getSort();
		Long max_distance = param.getMax_distance();
		
		//已自己为圆心
		GeoDistanceQueryBuilder builder = QueryBuilders.geoDistanceQuery(filed).point(lat, lon)
                .distance(max_distance, DistanceUnit.METERS);
		searchRequestBuilder.setPostFilter(builder);
        GeoDistanceSortBuilder order = SortBuilders.geoDistanceSort(filed,lat,lon)
                .unit(DistanceUnit.METERS)
                .order(sort==1?SortOrder.ASC:SortOrder.DESC);
        
        searchRequestBuilder.addSort(order);
