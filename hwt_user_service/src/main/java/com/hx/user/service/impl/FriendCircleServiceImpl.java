...
//	}
	...
	public void friendCircle_insert(FriendCircle friendCircle, LoginVo loginVo) throws Exception {
		...
		MongoService.insertIncId(HXSystemConfig.MONGO_DB_NAME_friendCircle,HXSystemConfig.MONGO_Collection_NAME_friendCircle, friendCircle,"friend_circle_id");
	...
	private Long[] StringToLong(String[] split){
		...
		if (split!=null && split.length>0){
			for (int i = 0; i < split.length; i++) {
				if (!ObjectHelper.isEmpty(split[i])){
	...
	public void friendCircle_delete(Long user_id, Long friend_circle_id) {
		...
		filterMap.put("friend_circle_id", friend_circle_id);
		...
		updateMap.put("is_hide", 1);
		//删除朋友圈
		MongoService.updateOne(HXSystemConfig.MONGO_DB_NAME_friendCircle, HXSystemConfig.MONGO_Collection_NAME_friendCircle, filterMap, updateMap);
	...
	public List<FriendCircleVo> friendCircle_query(Long user_id, Long last_friend_circle_id, Integer pageSize) {
		...
		//没有删除的
		condition1.add(new MongoQueryValue(MongoOperator.eq, "is_hide",0));
		...
		if (friendCircleJurisdictionVos!=null&&friendCircleJurisdictionVos.size()>=0){
			...
			for (int i=0; i< friendCircleJurisdictionVos.size(); i++) {
				...
				//没有删除的
				condition7.add(new MongoQueryValue(MongoOperator.eq, "is_hide",0));
...
//				MONGO_Collection_NAME_friendCircle_comment, sorts , null);
		...
		if (friendCircleJurisdictionVo==null){
			...
			if (hxUserConfig!=null){
				if (hxUserConfig.getFriend_circle_stranger_see_10()==1){
					...
					//没有删除的
					condition3.add(new MongoQueryValue(MongoOperator.eq, "is_hide",0));
					//好友的所有人可看
					condition3.add(new MongoQueryValue(MongoOperator.eq, "can_see_user_id","1"));
					if (last_friend_circle_id!=null&&last_friend_circle_id>0){
	...
	public FriendCircleComment friendCircle_comment(FriendCircleComment friendCircleComment) throws Exception {
		...
		if (friendCircleComment.getType()==2){
			...
			queryValue.add(new MongoQueryValue(MongoOperator.eq, "is_hide", 0));
		...
		MongoService.insertIncId(HXSystemConfig.MONGO_DB_NAME_friendCircle,HXSystemConfig.MONGO_Collection_NAME_friendCircle_comment, friendCircleComment,"comment_id");
	...
	public void friendCircle_comment_delete(Long user_id, Long comment_id, Long friend_circle_id) {
		...
		filterMap.put("user_id", user_id);
		filterMap.put("friend_circle_id", friend_circle_id);
		filterMap.put("comment_id", comment_id);
		Map<String, Object> updateMap = new HashMap<>();
		updateMap.put("is_hide", 1);
		MongoService.updateOne(HXSystemConfig.MONGO_DB_NAME_friendCircle, HXSystemConfig.MONGO_Collection_NAME_friendCircle_comment, filterMap, updateMap);
