...
public class ScenicSpotServiceImp implements ScenicSpotService {
	...
	public List<Map<String,Object>> queryScenic(String str, Integer pageIndex, Integer pageSize) {
		...
		try {
			...
			sortMap.put("dataTime",SortOrder.DESC);
...
//        conditionAnd.add(new MongoQueryValue(MongoOperator.eq, "city",city));
	...
	public Map<String,Object> querySpotInfo(Long spotId, Long user_id) {
		...
		String str = RedisCache.get(HwPrefix.ScenicSpotPrefix+spotId,3);
		if (ObjectHelper.isEmpty(str)){
			MongoQueryCondition conditionAnd = new MongoQueryCondition(LinkKey.and);
			conditionAnd.add(new MongoQueryValue(MongoOperator.eq,"spotId",spotId));
			resultMap = MongoService.findOne(HXSystemConfig.MONGO_Collection_NAME_hwt,HXSystemConfig.MONGO_Collection_NAME_ScenicSpot,conditionAnd,null);
			...
			//数据缓存5小时
			RedisCache.setpar(HwPrefix.ScenicSpotPrefix+spotId, JsonUtils.Bean2Json(map), 5*24*60*60*1000,3);
		} else {
		...
		if (resultMap==null){
			...
			resultMap =  (Map<String, Object>) map.get("scenic");
...
//				param.setSortField(sortMap);
	...
	@Override
	public List<Map<String, Object>> searchNearby(String search_key,Double longitude, Double latitude,Long distancem, Integer pageIndex,
			Integer pageSize) {
		...
		if(!ObjectHelper.isEmpty(search_key)){
			Map<String, Object> shouldStr = new HashMap<>();
			shouldStr.put("name", search_key);
			shouldStr.put("location", search_key);
			shouldStr.put("tar", search_key);
			shouldStr.put("area_code_name", search_key);
			param.setShouldStr(shouldStr);
		}
		...
		searchNearby.setFiled("point");
		searchNearby.setLat(latitude);
		searchNearby.setLon(longitude);
		searchNearby.setMax_distance(distancem);
		searchNearby.setSort(SearchNearby.ASC);
		param.setSearchNearby(searchNearby);
		List<Map<String,Object>> recordList = ElasticsearchUtils.searchNearbyListData(param).getRecordList();
		
		return recordList;
	}
	
	@Override
	public List<HxBigVideo> searchBigVideo(Map<String, Object> map) {
		List<HxBigVideo> list = hxUserVideoMapper.searchBigVideo(map);
