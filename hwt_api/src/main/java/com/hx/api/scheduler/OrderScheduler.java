...
public class OrderScheduler {
		...
		if (!ObjectHelper.isEmpty(query_unpaid)){
			for (HwOrderUnpaid hwOrderUnpaid : query_unpaid) {
				if (hwOrderUnpaid.getCicerone_id()!=0){
					sales_cicerone_update_workTime_new(hwOrderUnpaid);
	 ...
	 * @param hwOrder
	 */
	private void sales_cicerone_update_workTime_new(HwOrderUnpaid hwOrderUnpaid) {
		Long start_time = hwOrderUnpaid.getStart_time();
		Long end_time = hwOrderUnpaid.getEnd_time();
		
		hwPriceMapper.updateCiceroneWorkPriceByStartTimeAndEndTime(start_time, end_time, hwOrderUnpaid.getCicerone_id(), 0);
	}
