        ...
        {title:'操作', customHtml:
        	...
        	function(rowData,index){
				...
				if (rowData.is_legal!=1){
					...
					html+='<a onclick="adpower(this,1,'+rowData.bureau_user_id+',\'adminbureau/user/adpower_qurey\',\'用户授权---'+rowData.real_name+'\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>授权</div></a>'
					if (rowData.is_delete==1){
	        			html += '<a onclick="deleteAndRecoveryById(2,'+rowData.bureau_user_id+',\'adminbureau/user/delete\',\'确定要禁用用户---'+rowData.real_name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>删除</div></a>';
	        		}else{
	        			html += '<a onclick="deleteAndRecoveryById(1,'+rowData.bureau_user_id+',\'adminbureau/user/recovery\',\'确定要恢复用户---'+rowData.real_name+'？\')" href="javascript:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>恢复</div></a>';
	        		}
				}
