...
import com.hx.bureau.base.ResultCode;
 ...
 */
@WebFilter(urlPatterns = {"/adminbureau/*","/bureau/*"})
public class BureauFilter implements Filter{
	private static final Logger log = LoggerFactory.getLogger(BureauFilter.class);
	@Override
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		BureauUserCache bureauUserCache = BureauUserCacheTools.getSession(req);
    	if(ObjectHelper.isEmpty(bureauUserCache)) {
    		if(ObjectHelper.x_requested_with(req)){
    			//异步请求
    			if (url.endsWith("/list")||url.equalsIgnoreCase("/main")||url.equalsIgnoreCase("/bureau/index")){
        ...
        if(bureauUserCache.getIs_legal()==1){
        	chain.doFilter(request, response);
         	return;
        }
    	
        List<HxBureauModule> listModule = BureauUserCacheTools.getSessionForModule(req);
		...
		if(ObjectHelper.isEmpty(listModule)){
			...
			for (HxBureauModule adminModule : listModule) {
