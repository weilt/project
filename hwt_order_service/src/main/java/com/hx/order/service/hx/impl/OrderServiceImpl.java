...
//			hwOrder.setPayment(total);
			...
			//判断是否能下单
			cicerone_is_insert_order_new(hxCiceroneInfo.getUser_id(),redisLock,hwOrder,hxOrderInfo);
...
//			hxOrderInfo.setDec("导游服务"+day+"天");
	 ...
	 * @param redisLock 
	 */
	private void cicerone_is_insert_order_new(Long user_id, 
			RedisLock redisLock, HwOrderUnpaid hwOrder, HxOrderInfo hxOrderInfo) {
	//	BigDecimal total = ZZBigDecimalUtils.safeMultiply(everyday_price, day);
		...
		List<HwPrice> list = hwPriceMapper.selectCiceronePriceByStartTimeAndEndTime(getZeroPointTimestamps(start_time)
				,getZeroPointTimestamps(end_time), user_id);
		if (ObjectHelper.isNotEmpty(list)){
			for (int i = 0; i < list.size(); i++) {
				HwPrice hwPrice = list.get(i);
				if ("1".equals(hwPrice.getStatus().toString())){
					throw new RuntimeException("改导游"+hwPrice.getTime_str()+"已被其他人预约，请重新下单！");
				} else {
					total = ZZBigDecimalUtils.safeAdd(total, hwPrice.getAdult_price());
				}
				
				if (i==0){
					//开始天为单价
					hxOrderInfo.setUnit_price(hwPrice.getAdult_price());
				}
			}
		} else {
			throw new RuntimeException("该导游这几天不能被预约，请重新下单！");
		}
		...
		//更改接单状态
		hwPriceMapper.updateCiceroneWorkPriceByStartTimeAndEndTime(getZeroPointTimestamps(start_time)
				,getZeroPointTimestamps(end_time), user_id,1);
	}
	 ...
	 * @return
	 */
	private  Long getZeroPointTimestamps(Long time){
	...
	private BigDecimal returnPayment(HwOrderUnpaid hwOrder, String price, HxOrderInfo hxOrderInfo) {
		...
		//儿童单价
		BigDecimal bigDecimal = new BigDecimal(ObjectHelper.isEmpty(json2Bean.getChildPrice())?"0.00":json2Bean.getChildPrice());
		
		if (bigDecimal.compareTo(new BigDecimal("0.00"))==0&&children_num>0){
			throw new  RuntimeException("没有设置儿童价格，所以不能输入儿童数量！");
		}
		
		BigDecimal children = ZZBigDecimalUtils.safeMultiply(children_num, bigDecimal);
	...
	public void apply_refundable(Long user_id, Long order_id) throws Exception {
		...
		if (hwOrder!=null){
			if(!hwOrder.getUser_id().equals(user_id)){
	...
	private HwOrderRefund apply_sales(HwOrder hwOrder) throws Exception {
		...
		if (hwOrder.getStatus()==1){
			...
			if (hwOrder.getCicerone_id()!=0){
				...
				sales_cicerone_update_workTime_new(hwOrder);
			...
			if (hwOrder.getCicerone_id()!=0){
				Long start_time = hwOrder.getStart_time();
				Long oneDay = 5*24*60*60*1000L;  //5天毫秒数
				Long threeDay = 15*24*60*60*1000L;  //15天毫秒数
				...
				if(time<=0){
					...
					//导游退款为订单开始前不足24小时扣订单金额的50%
					penalty = ZZBigDecimalUtils.safeMultiply(hwOrder.getPayment(), 0.5);
					hwOrderRefund.setRefund_dec("导游退款为订单行程开始前1~5日取消订单，收取游客订单金额50%的违约金");
					remarks = "抠50%违约金";
					//账单
				...
				//	sentNotice.sendSystem(getUserImId(hwOrder.getCicerone_id()), "用户取消订单", "订单行程开始前1~5日取消订单，收取游客订单金额50%的违约金", null, 2, null, JsonUtils.Bean2Json(insertBill));
					sentNotice.sendSystem(getUserImId(hwOrder.getCicerone_id()), "金额到账通知", "您有一笔金额已到账", null, 2, null, JsonUtils.Bean2Json(insertBill));
				} else if (time>=oneDay && time < threeDay){
					//大于24小时小于72小时扣10%
					penalty = ZZBigDecimalUtils.safeMultiply(hwOrder.getPayment(), 0.3);
					hwOrderRefund.setRefund_dec("行程开始前6~15日取消订单，收取游客订单金额30%的违约金");
					remarks = "抠30%违约金";
					//账单
					Long user_id = hwOrder.getCicerone_id();
					hxUserWalletMapper.addBalanceByUserId(user_id, penalty);
					HxUserWalletBill insertBill = insertBill(hwOrder,penalty, 2, 6, "违约金收入", 1, 1);
					hxUserWalletBillMapper.insertSelective(insertBill);
//					sentNotice.sendSystem(getUserImId(hwOrder.getCicerone_id()), "用户取消订单", "订单行程开始前6~15日取消订单，收取游客订单金额30%的违约金", null, 2, null, JsonUtils.Bean2Json(insertBill));
					sentNotice.sendSystem(getUserImId(hwOrder.getCicerone_id()), "金额到账通知", "您有一笔金额已到账", null, 2, null, JsonUtils.Bean2Json(insertBill));
				} else {
					//大于15天不扣违约金
					hwOrderRefund.setRefund_dec("距离订单行程开始，大于15日不扣违约金");
					remarks = "全额退款";
				}
				//扣除违约金所剩金额
				...
				hxUserWalletMapper.orderRefundByUserId(user_id, safeSubtract,penalty);
				...
				sales_cicerone_update_workTime_new(hwOrder);
				//sentNotice.sendSystem(getUserImId(hwOrder.getCicerone_id()), "用户取消订单", "您有一个订单已被买家取消", null, 4, hwOrder.getOrder_id()+"", null);
				sentNotice.sendSystem(getUserImId(hwOrder.getCicerone_id()), "订单取消通知", "您有一个订单已被买家取消", null, 4, hwOrder.getOrder_id()+"", null);
				hwOrder.setStatus(5);
				hwOrder.setRefundable_time(currentTimeMillis);
			} else {
				//旅行社
	...
	private void sales_cicerone_update_workTime_new(HwOrder hwOrder) {
		Long start_time = hwOrder.getStart_time();
		Long end_time = hwOrder.getEnd_time();
		
		hwPriceMapper.updateCiceroneWorkPriceByStartTimeAndEndTime(start_time, end_time, hwOrder.getCicerone_id(), 0);
	}
	/**
	 * 将导游被预约改为可预约
	 * @param hwOrder
	...
	public void cicerone_delete(Long user_id, Long order_id) {
		...
		if (hwOrder==null){
			...
			if (!hwOrder.getCicerone_id().equals(user_id)){
				throw new RuntimeException("非本人不能操作");
			}else{
				if (!(hwOrder.getStatus()==3||hwOrder.getStatus()==5||hwOrder.getStatus()==6)){
					throw new RuntimeException("该订单还不能删除");
		...
		if (hwOrder==null){
			...
			if (!hwOrder.getCicerone_id().equals(user_id)){
				...
				//	sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "订单被确认", "您有一个订单被导游家确认", null, 4, hwOrder.getOrder_id()+"", null);
					sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "订单确认通知", "您有一个导游订单已被确认", null, 4, hwOrder.getOrder_id()+"", null);
			        ...
			        Map<String, Object> updateMap = new HashMap<>();
			    	updateMap.put("sales", (ObjectHelper.isEmpty(findOne2.get("sales"))?0:(int)findOne2.get("sales"))+1);
		...
		if (hwOrder==null){
			...
			if (!hwOrder.getCicerone_id().equals(user_id)){
				...
				if (hwOrder.getStatus()!=2){
					...
					sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "金额到账通知", "您有一笔金额已到账", null, 2, null, JsonUtils.Bean2Json(insertBill));
					...
					sentNotice.sendSystem(getUserImId(hwOrder.getCicerone_id()), "平台消息通知", "导游主动取消订单，3天内不能被下单！", null, 1, null, null);
				}
				
				//sales_cicerone_update_workTime(hwOrder);
				sales_cicerone_update_workTime_new(hwOrder);
				//sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "订单被取消", "您有一个订单被导游取消", null, 4, hwOrder.getOrder_id()+"", null);
				sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "订单取消通知", "您有一个导游订单已被取消", null, 4, hwOrder.getOrder_id()+"", null);
	...
	public void cicerone_refuse(Long user_id, Long order_id, int type) throws Exception {
		HwOrder hwOrder = hwOrderMapper.selectByPrimaryKey(order_id);
		if (hwOrder.getStatus()!=1){
			throw new RuntimeException("该订单不能拒绝了");
		}
		if (hwOrder==null){
			throw new RuntimeException("该订单不存在");
		}else{
		...
		if(type==0){
			...
			sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "订单退款通知", "对方导游没有在规定的时间确认", null, 4, hwOrder.getOrder_id()+"", null);
			sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "金额到账通知", "您有一笔金额已到账", null, 2, null, JsonUtils.Bean2Json(insertBill));
		}else{
			sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "订单退款通知", "您有一个导游订单已被拒绝", null, 4, hwOrder.getOrder_id()+"", null);
			sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "金额到账通知", "您有一笔金额已到账", null, 2, null, JsonUtils.Bean2Json(insertBill));
...
//			sentNotice.sendSystem(getUserImId(hwOrder.getUser_id()), "导游拒绝订单", "导游拒绝订单，所以全额退款", null, 2, null, JsonUtils.Bean2Json(insertBill));
	...
	public OrderDetailsVo qureyDetails(Long user_id, Long order_id) {
		...
		if(hwOrderVo.getUser_id().equals(user_id) || hwOrderVo.getCicerone_id().equals(user_id)){
