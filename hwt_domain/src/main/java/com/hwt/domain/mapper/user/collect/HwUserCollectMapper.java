...
public interface HwUserCollectMapper {
	 ...
	 * @return
	 */
    @Select("select collect_id, user_id, type, name_id, create_time from hw_user_collect where user_id = #{user_id} order by create_time desc limit #{startNum}, #{pageSize}")
	 ...
	 * @return
	 */
	@Select("select collect_id, user_id, type, name_id, create_time from hw_user_collect where user_id = #{user_id} and name_id = #{name_id} and type = #{type}")
