...
public interface HwOrderUsuallyUserMapper {
     ...
     * @return
     */
    @Select("select usually_id, user_id, name, eng_surname, eng_name, identity_type, identity_num, identity_effective, "
    +"birthday, sex, phone, is_own, create_time from hw_order_usually_user where user_id = #{user_id} order by is_own desc")
     ...
     */
    @Select("select usually_id, user_id, name, eng_surname, eng_name, identity_type, identity_num, identity_effective, "
    +"birthday, sex, phone, is_own, create_time  from hw_order_usually_user where user_id = #{user_id} and usually_id = #{usually_id}")
