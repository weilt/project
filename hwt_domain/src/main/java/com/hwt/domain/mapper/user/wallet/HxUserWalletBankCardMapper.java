...
public interface HxUserWalletBankCardMapper {
	 ...
	 * @return
	 */
    @Select("select  bank_card_id, name_id, name_type, card_num, username, bank_name, bank_address, image, "
    +"create_time from hx_user_wallet_bank_card where name_id = #{user_id} and name_type = 1")
	 ...
	 */
    @Select("select bank_card_id, name_id, name_type, card_num, username, bank_name, bank_address, image, "
    +"create_time  from hx_user_wallet_bank_card where card_num = #{card_num}")
